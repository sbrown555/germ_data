...
# # Defining functions for downloading new data
# processes = {'PRO_01':'Temp','PRO_02':'RH', 'PRO_03':'PAR_max', 'PRO_04':'PAR_umol','PRO_05':'CO2'}
...
#   processed_files = set()
#   list_df=[]
#   for chamber_folder in gc_data.iterdir():
#     if chamber_folder.is_dir() and 'other' not in str(chamber_folder).split('/')[-1]:
#       print(str(chamber_folder))
#       chamber = str(chamber_folder).split('/')[-1].split('_')[0][-1]
#       for process_folder in chamber_folder.iterdir():
#         if process_folder.is_dir():
#           for process in processes.keys():
#               for csv_file in process_folder.iterdir():
#                 if csv_file.is_file() and csv_file.suffix.lower() == '.csv' and process in str(csv_file):
#                   if string_id in str(csv_file):
...
#                     else:
#                       print(f'already processed {str(csv_file)}')
#   df = pd.concat(list_df, axis=0)
#   df.reset_index(inplace=True)
#   df['minute'] = df['datetime'].dt.strftime('%m/%d/%Y %H')
#   data = df.groupby(['minute', 'Chamber', 'Process']).agg({'Value':'mean'})
...
# Inputs and running the function
# ```{python}
# current_date = '20Aug25'
# download_date = '20250820'
# additional_file_info = ''
# save_figure = True

# time_offset = [pd.Timedelta(days=0), pd.Timedelta(days=0)]
# folder_name = f'{download_date}_Chamber_Data'
# data = data_from_date(folder_name, time_offset, actual=True)
# data_sp = data_from_date(folder_name, time_offset, actual = False)
